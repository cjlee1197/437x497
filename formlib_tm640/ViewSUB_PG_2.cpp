...
//	SelectNo		=GetDBValue("SYSX_OTHERS_OTHERS_INT_RESERVED51").lValue;		// 步驟
//	ActionType = GetDBValue("SYSX_OTHERS_OTHERS_INT_RESERVED52").lValue;		// 類型
//	printf("SelectNo%d ActionType=%d\n",SelectNo,ActionType);
	char 	pDataID[256];
	
	SetVisible( pwndimageBackGround1, 1,0x0031);
	if(pwndSelectEditNUM != NULL)			// 編號 NUM
	{
...
WORD OnKeyA(CtmWnd* pwndSender, WORD wKey)
{
	...
	{
		::PutCommand("SUB_PG_2_LABEL2.txt");
	}
	else if (wKey == 0x0006)
	{
		::PutCommand("SUB_PG_2_GOTO2.txt");
	}
	else if (wKey == 0x0007)
	{
		::PutCommand("SUB_PG_2_VALVE2.txt");
	}
