...
void	OnUpdateA(CtmWnd* pwndSender)
{
	...
	iSelectIndex = GetDBValue("SYSX_OTHERS_OTHERS_INT_RESERVED46").lValue; // 更新偵測 選擇是否有在ViewPG_3被改變
...
WORD	OnMouseUp(CtmWnd* pwndSender, WORD wIDControl)
{
	...
	else if(pwnd == pwndSelectEditNUM) // 軸動作選擇
	{
		...
		{
			MsgBoxCall("EditWindow_Wait2.txt");
		}
		else if(ActionType == Action_Permit) // Permit
		{
			MsgBoxCall("EditWindow_Permit2.txt");
		}
		else if (ActionType == Action_Tag) // 標籤
		{
			MsgBoxCall("EditWindow_Tag2.txt");
		}
		else if (ActionType == Action_Goto) // 跳轉
		{
			MsgBoxCall("EditWindow_Goto2.txt");
		}
		else if (ActionType == Action_Valve) // 閥門
		{
			MsgBoxCall("EditWindow_Valve2.txt");
		}
		else if (ActionType == Action_Detect) // 檢測
		{
			MsgBoxCall("EditWindow_Detect2.txt");
		}
	}
