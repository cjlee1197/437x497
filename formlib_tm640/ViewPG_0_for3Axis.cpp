...
BOOL	OnCreateA(CtmWnd* pwndSender)
{
	...
	{	//場宎隅砱 初始定義
		...
		//SetDBValue("MACHINE_PROFILE_STEPNUM", 0);		//杅講ь錨 數量清零
...
//						break;
				...
				//if(ACTIONNUM <= 5) // cjlee 五軸 2019/4/6 下午 05:44:41
				if(ACTIONNUM <= 3 || ACTIONNUM >= 5) // cjlee 三軸2019/4/6 下午 05:44:41
				{
				...
				//else if(ACTIONNUM == 6) // cjlee 五軸
				else if(ACTIONNUM == 4) // cjlee 三軸 2019/4/6 下午 05:44:41
				{
					if(wActPara2)
...
*/
	...
	for (int i = 0; i < iEditNo; i++)	//鳳��唗瘍 獲參序號
	{
		...
		else
		{
			...
			if(No>EditedStepNum)value=5; // 未編輯的動作
...
//			printf("ACTION_STEP_Pre=%d\n",ACTION_STEP);
	...
	if(u_wPickerOPSatus != STATE_FULLAUTO) //赻雄狟癹秶 自動下限制
	{
...
WORD	OnMouseUp(CtmWnd* pwndSender, WORD wIDControl)
{
 	...
 	else if(pwnd ==pwndButton_Back)
	{
 	...
 	else if(pwnd == pwndBtnHome) // Home // cjlee 2019/4/17 下午 07:16:53
	{
		::PutCommand("Over.txt");
	}  
...
void	Clear()								//ь諾垀衄杅擂
{
	...
	for(int i=0;i<7;i++) // Set ACTION_STEP 前7步
	{
		memset(pDataID, 0 ,sizeof(pDataID));
		sprintf(pDataID,"MACHINE_PROFILE_NUM%d_ACTION_STEP",i+1);
...
//						break;
				...
				//if(ACTIONNUM <= 5) // 五軸
				if(ACTIONNUM <= 3 || ACTIONNUM >= 5) // 三軸 cjlee 2019/4/6 下午 05:41:56
				{
					sprintf(pDataID5,"PICKER_DESCRIBE_AXIS_1"); // "以"
					sprintf(pDataID6,"%3d",wActPara2);
					sprintf(pDataID7,"PICKER_DESCRIBE_AXIS_2"); // "的速度移動到"
				...
				//else if(ACTIONNUM == 6) // 五軸
				else if(ACTIONNUM == 4)  // 三軸cjlee 2019/4/6 下午 05:41:56
				{
					if(wActPara2)
						sprintf(pDataID4,"PICKER_REMOTE_O_14");
					else
