...
+==========================================================================*/
#ifdef	D_32BIT
extern 	unsigned long  *g_Rows[tmMAXVIDEO_Y];		//gui.cpp
#else
extern 	unsigned short  *g_Rows[tmMAXVIDEO_Y];		//gui.cpp
#endif
/*==========================================================================+
|          Function implementation                                          |
+==========================================================================*/

int CtmGif::maskTbl[] = {0x0000, 0x0001, 0x0003, 0x0007, 0x000f, 0x001f, 0x003f, 0x007f,
			  0x00ff, 0x01ff, 0x03ff, 0x07ff, 0x0fff, 0x1fff, 0x3fff, 0x7fff};

gif_bitmap_callback_vt CtmGif::m_Bitmap_callbacks = {
		bitmap_create,
		bitmap_destroy,
		bitmap_get_buffer,
		bitmap_set_opaque,
...
*/
void CtmGif::gif_create(gif_animation *gif, gif_bitmap_callback_vt *bitmap_callbacks) {
...
*/
gif_result CtmGif::gif_initialise(gif_animation *gif, size_t size, unsigned char *data) {
...
/**	Updates the sprite memory size
	...
	@return GIF_INSUFFICIENT_MEMORY for a memory error
		GIF_OK for success
*/
gif_result CtmGif::gif_initialise_sprite(gif_animation *gif, unsigned int width, unsigned int height) {
	...
	*/
	assert(gif->bitmap_callbacks.bitmap_create);
	if ((buffer = (bitmap*)(gif->bitmap_callbacks.bitmap_create(max_width, max_height))) == NULL)
		return GIF_INSUFFICIENT_MEMORY;
...
/**	Attempts to initialise the next frame
	...
	@return GIF_INSUFFICIENT_DATA for insufficient data to do anything
		...
		GIF_WORKING for successful decoding if more frames are expected
*/
gif_result CtmGif::gif_initialise_frame(gif_animation *gif) {
	int frame;
	gif_frame *temp_buf;
...
*/
gif_result CtmGif::gif_initialise_frame_extensions(gif_animation *gif, const int frame) {
	unsigned char *gif_data, *gif_end;
	int gif_bytes;
...
*/
gif_result CtmGif::gif_decode_frame(gif_animation *gif, unsigned int frame) {
...
/**	Skips the frame's extensions (which have been previously initialised)
	...
	@return GIF_INSUFFICIENT_FRAME_DATA for insufficient data to complete the frame
		GIF_OK for successful decoding
*/
gif_result CtmGif::gif_skip_frame_extensions(gif_animation *gif) {
	unsigned char *gif_data, *gif_end;
	int gif_bytes;
	unsigned int block_size;
...
unsigned int CtmGif::gif_interlaced_line(int height, int y) {
...
*/
void CtmGif::gif_finalise(gif_animation *gif) {
 ...
 */
void CtmGif::gif_init_LZW(gif_animation *gif) {
	int i;
...
bool CtmGif::gif_next_LZW(gif_animation *gif) {
	int code, incode;
	int block_size;
	int new_code;
...
int CtmGif::gif_next_code(gif_animation *gif, int code_size) {
...
|	remark		把圖片讀入內存，準備顯示									|
+==========================================================================*/
int		CtmGif::Create(char *FileName)
{
	int		FileHandle;
	gif_result 		Code;
	...
	gif_create(&m_Gif, &m_Bitmap_callbacks);
	
	m_Data = Load_File(FileName, &m_dwSize);
	
	do {
		Code = gif_initialise(&m_Gif, m_dwSize, m_Data);
		if (Code != GIF_OK && Code != GIF_WORKING)
			return tmBMP_ERROR_FILE;
	} while (Code != GIF_OK);
	
	m_pBuff = (char *)malloc(m_Gif.width 
								* m_Gif.height 
								* g_infoDrv.nBits >> 3);
	
//	for(int y = 0; y < g_infoDrv.nY; y++)
//		memcpy(m_Rows[y], g_Rows[y], g_infoDrv.nX);	
	
	return 0;
}


int		CtmGif::SetRegion(RECT rect)
{
	if(m_rectBuffer)
	{
		free(m_rectBuffer);
	...
	#ifdef D_32BIT
	m_rectBuffer=static_cast<DWORD*>(malloc((rect.bottom-rect.top+1)*(rect.right-rect.left+1)*sizeof(DWORD)));
	m_Rows=static_cast<DWORD**>(malloc((rect.bottom-rect.top+1)*sizeof(DWORD*)));
	#else
	m_rectBuffer=static_cast<WORD*>(malloc((rect.bottom-rect.top+1)*(rect.right-rect.left+1)*sizeof(WORD)));
	m_Rows=static_cast<WORD**>(malloc((rect.bottom-rect.top+1)*sizeof(WORD*)));
	#endif
	for(int y=rect.top; y<=rect.bottom; y++)
	{
		m_Rows[y-rect.top]=m_rectBuffer+(y-rect.top)*(rect.right-rect.left+1);
		memcpy(m_Rows[y-rect.top],g_Rows[y]+rect.left,rect.right-rect.left+1);	
	}	
}
...
|	remark		設置圖片中某種顏色不顯示									|
+==========================================================================*/
int		CtmGif::SetKeyColor(DWORD Color)
{
	#ifdef	D_32BIT
...
+==========================================================================*/
int		CtmGif::Show(RECT rect, WORD AFlag, WORD wFrameIndex)
{
	short 	Temp;
	int		i,j;
	int		Row, Col;
	...
	WORD*	pBuff;
	#endif
	if(wFrameIndex >= m_Gif.frame_count) wFrameIndex = m_Gif.frame_count - 1;
	if(m_wOldFrameIndex != wFrameIndex)
	{
		DecodeFrameTOBuffer(wFrameIndex);
		m_wOldFrameIndex = wFrameIndex;
	}
	...
	if(rect.right - rect.left + 1 > m_Gif.width)	
		Col	= m_Gif.width;
	else
		Col	= rect.right - rect.left + 1;
	
	#ifdef	D_32BIT
	if(rect.bottom - rect.top + 1 > m_Gif.height)
		{
		Row	= m_Gif.height;
		pBuff	= (DWORD *)m_pBuff;
		}
		...
		pBuff	= (DWORD *)(m_pBuff + (m_Gif.height - Row) * m_Gif.width * sizeof(DWORD));
		}
	...
	if(rect.bottom - rect.top + 1 > m_Gif.height)
		...
		{
		Row	= rect.bottom - rect.top + 1;
		pBuff	= (WORD *)(m_pBuff + (m_Gif.height - Row) * m_Gif.width * sizeof(WORD));
		}
	...
	if(AFlag & tmBMP_KEYCOLOR)
	{
		for(i = Row - 1; i >= 0; i --)
		{
			for(j = 0; j < Col; j ++)
			{
				...
				{
					if (m_pDoubleBuffer != NULL) 
						pTemp[i][j] = m_Rows[rect.top + i][rect.left + j];
						
				}
				
				pBuff	++;
			}
			pBuff	+=	m_Gif.width	- Col;
		}
	}
	else
	{
...
void		CtmGif::SetDoubleBuffer(CtmGDIDoubleBuffer* pDoubleBuffer)
{
	m_pDoubleBuffer = pDoubleBuffer;
}

gif_animation*	CtmGif::GetInfo()
{
	return &m_Gif;	
}

void *		CtmGif::bitmap_create(int width, int height)
{
	return calloc(width * height, 4);
}


void 		CtmGif::bitmap_set_opaque(void *bitmap, bool opaque)
{
	(void) opaque;  /* unused */
	assert(bitmap);
}
...
bool 		CtmGif::bitmap_test_opaque(void *bitmap)
{
	assert(bitmap);
	return false;
}
...
unsigned char* CtmGif::bitmap_get_buffer(void *bitmap)
{
	assert(bitmap);
...
void 		CtmGif::bitmap_destroy(void *bitmap)
{
	assert(bitmap);
	free(bitmap);
}
...
void 		CtmGif::bitmap_modified(void *bitmap)
{
	assert(bitmap);
	return;
}

BYTE* 		CtmGif::Load_File(char* pszPath, DWORD* pdwSize)
{
	FILE *fd;
	struct stat sb;
	BYTE *pBuffer;
...
void 		CtmGif::DecodeFrameTOBuffer(WORD wFrameIndex)
{
	int nRow, nCol;
	int   i = 0;
	#ifdef	D_32BIT
	...
	pWord	= (WORD *)m_pBuff;
	#endif
	gif_decode_frame(&m_Gif, wFrameIndex);
	BYTE* bImage = (BYTE*)m_Gif.frame_image;
	
	for (nRow = m_Gif.height - 1; nRow >= 0; nRow--) 
	{
		for (nCol = 0; nCol < m_Gif.width; nCol++) 
		{
			i = (nRow * m_Gif.width + nCol) * 4;
			
			#ifdef	D_32BIT
			*pWord  = RGB565TORGB888(RGB2PIXEL(bImage[i], bImage[i + 1], bImage[i + 2]));
			#else
			*pWord	= RGB2PIXEL(bImage[i], bImage[i + 1], bImage[i + 2]);
			#endif
			pWord++;
		}
	}
