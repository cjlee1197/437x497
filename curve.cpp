...
void	CCurve::SetPropertys()
{
	...
	SetPropInfo((char *)"tableline",	tmBOOL,		&m_bTableLine);			//James add 2011/7/13 下午 05:19:16
	SetPropInfo((char *)"xRulercolor",	tmINT,		&m_nXRulerLineColor);	//James add 2011/7/13 下午 05:19:16
	SetPropInfo((char *)"yRulercolor",	tmINT,		&m_nYRulerLineColor);	//James add 2011/7/13 下午 05:19:16
...
//	printf("m_nXRulerLineColor=%x m_nYRulerLineColor=%x \n", m_nXRulerLineColor, m_nYRulerLineColor);
	m_pDoubleBuffer->SetColorInBuffer(m_nYRulerLineColor);			//設置Y標尺的顏色 James add 2011/7/13 下午 05:33:37
	...
	m_pDoubleBuffer->DrawLineToInBuffer(m_rectCoordinate.left + 2, m_rectCoordinate.top + 8);    
	m_pDoubleBuffer->MoveToInBuffer(m_rectCoordinate.left, m_rectCoordinate.bottom);        
	m_pDoubleBuffer->SetColorInBuffer(m_nXRulerLineColor); 			//設置X標尺的顏色 James add 2011/7/13 下午 05:33:37
	...
	if(m_bTableLine)				//是否畫網格 jame add 2011/7/13 下午 05:32:30  
	{
...
void	CtmCurveEx::SetPropertys()
{
    ...
    SetPropInfo((char *)"showgrid",		tmBOOL,		&m_bShowGrid);			//Gerry add 2011/9/17 下午 03:22:15
...
void	CtmCurveEx::DrawRuleMark(int nLineIndex)
{
	...
	if (GetRuleType() & RULE_HORIZONTAL)		//水平標尺
	{	
	...
	else if (GetRuleType() & RULE_VERTICAL)		//垂直標尺
	{
		if (GetPointLocation(&point) >= 0)
...
void	CtmCurveEx::StretchCurve(double dRatio)	//縮放座標系
{
	m_bPropertyChanged=TRUE;
...
void	CtmCurveEx::MoveCoordinate(double dXDelta, double dYDelta)	//移動座標系平面
{
...
void	CtmCurveEx::ResetCoordinate()	//重置座標系位置
{
	m_bPropertyChanged=TRUE;
	m_nOpMode = CURVE_OPERATION_NONE;
...
void	CtmCurveEx2::StretchCurve(double dRatio)	//縮放座標系
{
...
void	CtmCurveEx2::MoveCoordinate(double dXDelta, double dYDelta)	//移動座標系平面
{
    m_nOpMode = CURVE_OPERATION_MOVE_COORDINATE;
...
void	CtmCurveEx2::ResetCoordinate()	//重置座標系位置
{
	m_bPropertyChanged=TRUE;
    m_nOpMode = CURVE_OPERATION_NONE;
