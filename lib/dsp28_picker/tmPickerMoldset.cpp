#include	"../../tmdebug.h"
#include	"../../database.h"
#include	"tmPickerMoldset.h"


IMPLEMENT_DYNCREATE(CtmPickerMoldset, CtmDsp28Protocol)
//IMPLEMENT_DYNCREATE(CtmDsp28CommonDiagWrite, CtmPickerMoldset)

CtmPickerMoldset::CtmPickerMoldset(): CtmDsp28Protocol()
{
	
}

CtmPickerMoldset::~CtmPickerMoldset()
{
	
}


int		CtmPickerMoldset::CreateSelf()
{
...
int		CtmPickerMoldset::ComposeProtocol	(void* pOutData,	void* pInData, int nInputLen, void* pEx)
{
	//printf("nInputLen=%d \n", nInputLen);
	...
	for (int i = 0; i < nInputLen; i++)
	{
		...
		{
		//	*(WORD*)pTemp	= plID[i];	
			memcpy(pTemp, &plID[i], sizeof(plID[i]));	                     //21040416 add		
...
int		CtmPickerMoldset::ParseProtocol	(void* pInData,		int nInputLen)
{
